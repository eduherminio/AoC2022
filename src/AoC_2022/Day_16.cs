...
public partial class Day_16 : BaseDay
{
    ...
    public override ValueTask<string> Solve_2()
    {
        ...
        while (stack.Count > 0)
        {
            ...
            // Optimize search to avoid following 'dead paths' by using Dijkstra distance from source as a 'worst case scenario' that ensures a correct solution
            // Worst case scenario because in case a valve is on the way to another, the distance would be smaller.
            // I think it could however fail in the followin scenarios (so probably just been lucky here):
            // Valve1 -------------- CurrentValve---------------- Valve2        where time left > 16
            //            <5>                           <6>
            var valvesLeft = valvesThatReleasePressure.ExceptBy(current.OpenNodes, node => node.Id).Except(new[] { current.Nodes.Last().Node }).ToList();
